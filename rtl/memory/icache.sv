...
// - 返回按请求先后顺序保序；如果两个请求同拍出现，则地址更小的那个更早。
// - 更晚的请求不能早于更早的请求返回；如果两者同拍都 ready，可以同拍一起返回。
//
// 实现：
...
module icache #(
    ...
    localparam logic PREFETCH_ENABLED = (PREFETCH_LINES_AHEAD > 0);
    localparam logic PREFETCH_LIMIT_ENABLED = (PREFETCH_MAX_OUTSTANDING > 0);
    localparam logic [PREFETCH_CNT_W-1:0] PREFETCH_MAX_OUTSTANDING_COUNT =
        PREFETCH_CNT_W'(PREFETCH_MAX_OUTSTANDING);
    ...
    always_comb begin
        ...
        prefetch_issue_allowed =
            prefetch_stream_valid &&
            PREFETCH_LIMIT_ENABLED &&
            (active_prefetch_mshr_cnt < PREFETCH_MAX_OUTSTANDING_COUNT);
    ...
    always_comb begin
        ...
        if (active_now[0] && active_now[1]) begin
            if (port0_older_now) begin
                if (port_ready_all_valid[0]) begin
                    port_resp_fire[0] = 1'b1;
                    port_resp_fire[1] = port_ready_all_valid[1];
                end
            end else begin
                if (port_ready_all_valid[1]) begin
                    port_resp_fire[1] = 1'b1;
                    port_resp_fire[0] = port_ready_all_valid[0];
                end
            end
        end else begin
            port_resp_fire[0] = active_now[0] && port_ready_all_valid[0];
            port_resp_fire[1] = active_now[1] && port_ready_all_valid[1];
        end
    end
    ...
    generate
        if (PREFETCH_LINES_AHEAD > 0) begin : gen_demand_prefetch
            always_comb begin
        ...
        end else begin : gen_no_demand_prefetch
            always_comb begin
                demand_prefetch_valid = 1'b0;
                demand_prefetch_base_addr = 32'b0;
                demand_prefetch_cursor_addr = 32'b0;
